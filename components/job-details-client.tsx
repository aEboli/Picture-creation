...
export function JobDetailsClient({
  ...
  async function handleFeishuResync() {
    ...
    try {
      ...
      const safeDetailsResponse = await fetch(`/api/jobs/${details.job.id}`, { cache: "no-store" });
      if (safeDetailsResponse.ok) {
        const safeDetails = (await safeDetailsResponse.json()) as JobDetails;
