...
test("job details polling is visibility-aware and single-flight", () => {
  const detailsContent = readSource("components", "job-details-client.tsx");

  assert.match(detailsContent, /const VISIBLE_POLL_INTERVAL_MS = 5000;/);
  assert.match(detailsContent, /const HIDDEN_POLL_INTERVAL_MS = 15000;/);
  assert.match(detailsContent, /document\.visibilityState/);
  assert.match(detailsContent, /document\.addEventListener\("visibilitychange"/);
  assert.match(detailsContent, /jobDetailsRequestInFlightRef/);
  assert.match(detailsContent, /pendingRefreshRef/);
});

test("feishu resync reuses centralized job detail refresh", () => {
  const detailsContent = readSource("components", "job-details-client.tsx");

  assert.match(detailsContent, /await refreshJobDetails\(\);/);
});

test("review updates also trigger centralized job detail refresh", () => {
  const detailsContent = readSource("components", "job-details-client.tsx");

  assert.match(detailsContent, /void refreshJobDetails\(\);/);
});
