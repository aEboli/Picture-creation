...
test("settings source uses compact L-shaped desktop cards and dual global footer actions", () => {
  const settingsForm = read(sourcePath("components", "settings-form.tsx"));
  const cssContent = read(sourcePath("app", "ui-ux-pro-max.css"));

  assert.match(settingsForm, /settings-l-layout/);
  assert.match(settingsForm, /settings-gemini-grid/);
  ...
  assert.doesNotMatch(settingsForm, /settings-hero/);
  assert.doesNotMatch(settingsForm, /settings-hero-title/);
  ...
  assert.match(cssContent, /\.settings-gemini-grid\s*\{/);
  assert.match(cssContent, /\.settings-field-span-3\s*\{/);
  assert.match(cssContent, /\.settings-l-layout\s*\{[\s\S]*grid-template-areas:\s*"gemini feishu"\s*"storage feishu";/);
  ...
  assert.doesNotMatch(cssContent, /\.settings-hero\s*\{/);
  assert.doesNotMatch(cssContent, /\.settings-hero-title\s*\{/);
  assert.match(
    cssContent,
  ...
  assert.match(
    cssContent,
    /@media \(max-width:\s*1023px\)\s*\{[\s\S]*\.settings-gemini-grid\s*\{[\s\S]*grid-template-columns:\s*repeat\(2,\s*minmax\(0,\s*1fr\)\);/,
  );
  assert.match(
    cssContent,
  ...
  assert.match(
    cssContent,
    /@media \(max-width:\s*720px\)\s*\{[\s\S]*\.settings-gemini-grid\s*\{[\s\S]*grid-template-columns:\s*minmax\(0,\s*1fr\);/,
  );
  assert.match(
    cssContent,
