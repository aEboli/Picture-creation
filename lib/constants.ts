...
import type { ImageType, SelectOption } from "./types.ts";

export const APP_NAME = "Picture-creation";
...
const SUPPORTED_RESOLUTION_VALUES = new Set(RESOLUTIONS.map((option) => option.value));
const LEGACY_RESOLUTION_TO_SUPPORTED: Record<string, string> = {
  "0.5K": "1K",
  "512px": "1K",
};

function normalizeResolutionValue(value: string | null | undefined): string | null {
  if (!value) {
    return null;
  }
  ...
  const migrated = LEGACY_RESOLUTION_TO_SUPPORTED[trimmed] ?? trimmed;
  return SUPPORTED_RESOLUTION_VALUES.has(migrated) ? migrated : null;
}

export function normalizeSelectedResolutions(values: readonly string[] | null | undefined): string[] {
  const normalized: string[] = [];
  const seen = new Set<string>();

  for (const value of values ?? []) {
    const nextValue = normalizeResolutionValue(value);
  ...
  return normalized.length > 0 ? normalized : ["1K"];
}
