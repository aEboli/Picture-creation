...
test("create form source keeps all workbench areas visible on the same surface", () => {
  ...
  assert.match(uiContent, /create-area-panel-settings/);
  assert.match(uiContent, /create-output-panel/);
  assert.match(uiContent, /browserApiSettings/);
  assert.match(uiContent, /temporaryProvider:/);
  ...
  assert.match(cssContent, /create-area-panel-stage\.is-settings/);
  assert.match(cssContent, /create-output-panel/);
  assert.doesNotMatch(cssContent, /create-api-settings-grid/);
  assert.match(cssContent, /\.create-area-panels\s*\{[\s\S]*display: grid;[\s\S]*"assets assets assets assets brief brief brief settings settings settings submit submit"[\s\S]*"assets assets assets assets market market market settings settings settings submit submit"[\s\S]*gap: 0;[\s\S]*overflow: hidden;/);
  ...
  assert.match(cssContent, /\.create-area-panel-stage\.is-settings\s*\{\s*grid-area: settings;/);
  ...
  assert.match(cssContent, /\.create-workspace\.is-reference-mode \.create-area-panel-stage\.is-assets\s*\{[\s\S]*grid-template-rows: minmax\(0, 1fr\) minmax\(0, 1fr\);/);
  assert.match(cssContent, /\.create-workspace\.is-reference-mode \.create-area-panel-stage\.is-assets \.create-source-panel,[\s\S]*\.create-workspace\.is-reference-mode \.create-area-panel-stage\.is-assets \.create-reference-panel\s*\{[\s\S]*height: 100%;[\s\S]*min-height: 0;[\s\S]*overflow: hidden;/);
  assert.match(cssContent, /\.create-workspace\.is-reference-mode \.create-area-panel-stage\.is-assets \.preview-stage,[\s\S]*\.create-workspace\.is-reference-mode \.create-area-panel-stage\.is-assets \.preview-stage-image\s*\{[\s\S]*min-height: 0;/);
